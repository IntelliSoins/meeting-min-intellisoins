...
export function RecordingSettings({ onSave }: RecordingSettingsProps) {
  const t = useTranslations('settings.recording');
  ...
  const handleNotificationToggle = async (enabled: boolean) => {
    try {
      ...
      toast.error(t('failedToSavePreference'));
    }
  };
  ...
  const savePreferences = async (prefs: RecordingPreferences) => {
    ...
    try {
      ...
      toast.error(t('devicePreferencesFailed'), {
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 border rounded-lg bg-blue-50">
            <div className="text-sm text-blue-800">
              <strong>{t('fileFormat')}</strong> {preferences.file_format.toUpperCase()} files
            </div>
            <div className="text-xs text-blue-600 mt-1">
              {t('fileFormatNote', { format: preferences.file_format })}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between p-4 border rounded-lg">
        <div className="flex-1">
          ...
          <div className="text-sm text-gray-600">
            {t('recordingStartNotificationDesc')}
          </div>
        </div>
        <Switch
